...
Elm.Minesweeper.Tile = Elm.Minesweeper.Tile || {};
Elm.Minesweeper.Tile.make = function (_elm) {
   "use strict";
   _elm.Minesweeper = _elm.Minesweeper || {};
   _elm.Minesweeper.Tile = _elm.Minesweeper.Tile || {};
   if (_elm.Minesweeper.Tile.values)
   return _elm.Minesweeper.Tile.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var classFor = function (tile) {
      var addExposed = F2(function (tile,$class) {
         return tile.isExposed ? A2($Basics._op["++"],
         $class,
         " exposed") : $class;
      });
      var addMine = F2(function (tile,$class) {
         return tile.isMine ? A2($Basics._op["++"],
         $class,
         " mine") : $class;
      });
      var addMarked = F2(function (tile,$class) {
         return tile.isMarked ? A2($Basics._op["++"],
         $class,
         " marked") : $class;
      });
      return A2(addExposed,
      tile,
      A2(addMine,tile,A2(addMarked,tile,"tile")));
   };
   var textFor = function (tile) {
      return tile.isExposed && $Basics.not(tile.isMine) ? $Basics.toString(tile.numberOfAdjacentMines) : "";
   };
   var mark = function (tile) {
      return _U.update(tile,{isMarked: true});
   };
   var expose = function (tile) {
      return _U.update(tile,{isExposed: true});
   };
   var Tile = F5(function (a,b,c,d,e) {
      return {id: a
             ,numberOfAdjacentMines: b
             ,isMine: c
             ,isMarked: d
             ,isExposed: e};
   });
   var $new = function (id) {
      return A5(Tile,id,0,false,false,false);
   };
   return _elm.Minesweeper.Tile.values = {_op: _op
                                         ,Tile: Tile
                                         ,$new: $new
                                         ,expose: expose
                                         ,mark: mark
...
Elm.Minesweeper = Elm.Minesweeper || {};
Elm.Minesweeper.Board = Elm.Minesweeper.Board || {};
Elm.Minesweeper.Board.make = function (_elm) {
   "use strict";
   _elm.Minesweeper = _elm.Minesweeper || {};
   _elm.Minesweeper.Board = _elm.Minesweeper.Board || {};
   if (_elm.Minesweeper.Board.values)
   return _elm.Minesweeper.Board.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Minesweeper$Tile = Elm.Minesweeper.Tile.make(_elm),
   $Random = Elm.Random.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var addAdjacentMineValues = function (tiles) {
      var neighbors = function (tile) {
         var isNWCorner = function (tile) {
            return _U.eq(tile.id,0);
         };
         var boardSize = $Basics.round($Basics.sqrt($Basics.toFloat($Array.length(tiles))));
         var isNECorner = function (tile) {
            return _U.eq(tile.id,boardSize - 1);
         };
         var isSWCorner = function (tile) {
            return _U.eq(tile.id,(boardSize - 1) * boardSize);
         };
         var isSECorner = function (tile) {
            return _U.eq(tile.id,boardSize * boardSize - 1);
         };
         var isTopRow = function (tile) {
            return _U.eq(tile.id / boardSize | 0,0);
         };
         var isBottomRow = function (tile) {
            return _U.eq(tile.id / boardSize | 0,boardSize - 1);
         };
         var isLeftMostRow = function (tile) {
            return _U.eq(A2($Basics.rem,tile.id,boardSize),0);
         };
         var isRightMostRow = function (tile) {
            return _U.eq(A2($Basics.rem,tile.id,boardSize),
            boardSize - 1);
         };
         var neighborIndexes = isNWCorner(tile) ? _U.list([tile.id + 1
                                                          ,tile.id + boardSize
                                                          ,tile.id + boardSize + 1]) : isNECorner(tile) ? _U.list([tile.id - 1
                                                                                                                  ,tile.id + boardSize
                                                                                                                  ,tile.id + boardSize + 1]) : isSWCorner(tile) ? _U.list([tile.id - boardSize
                                                                                                                                                                          ,tile.id - boardSize + 1
                                                                                                                                                                          ,tile.id + 1]) : isSECorner(tile) ? _U.list([tile.id - boardSize - 1
                                                                                                                                                                                                                      ,tile.id - boardSize
                                                                                                                                                                                                                      ,tile.id - 1]) : isTopRow(tile) ? _U.list([tile.id - 1
                                                                                                                                                                                                                                                                ,tile.id + 1
                                                                                                                                                                                                                                                                ,tile.id + boardSize - 1
                                                                                                                                                                                                                                                                ,tile.id + boardSize
                                                                                                                                                                                                                                                                ,tile.id + boardSize + 1]) : isBottomRow(tile) ? _U.list([tile.id - boardSize - 1
                                                                                                                                                                                                                                                                                                                         ,tile.id - boardSize
                                                                                                                                                                                                                                                                                                                         ,tile.id - boardSize + 1
                                                                                                                                                                                                                                                                                                                         ,tile.id - 1
                                                                                                                                                                                                                                                                                                                         ,tile.id + 1]) : isLeftMostRow(tile) ? _U.list([tile.id - boardSize
                                                                                                                                                                                                                                                                                                                                                                        ,tile.id - boardSize + 1
                                                                                                                                                                                                                                                                                                                                                                        ,tile.id + 1
                                                                                                                                                                                                                                                                                                                                                                        ,tile.id + boardSize
                                                                                                                                                                                                                                                                                                                                                                        ,tile.id + boardSize + 1]) : isRightMostRow(tile) ? _U.list([tile.id - boardSize - 1
                                                                                                                                                                                                                                                                                                                                                                                                                                    ,tile.id - boardSize
                                                                                                                                                                                                                                                                                                                                                                                                                                    ,tile.id - 1
                                                                                                                                                                                                                                                                                                                                                                                                                                    ,tile.id + boardSize - 1
                                                                                                                                                                                                                                                                                                                                                                                                                                    ,tile.id + boardSize]) : _U.list([tile.id - boardSize - 1
                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ,tile.id - boardSize
                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ,tile.id - boardSize + 1
                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ,tile.id - 1
                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ,tile.id + 1
         ...
         function (tile) {
            return A2($List.member,tile.id,neighborIndexes);
         },
         tiles);
      };
      var calculateNeighboringMines = function (tile) {
         return $Array.length(A2($Array.filter,
         function (_) {
            return _.isMine;
         },
         neighbors(tile)));
      };
      var populateAdjacentMines = function (tile) {
         return _U.update(tile,
         {numberOfAdjacentMines: calculateNeighboringMines(tile)});
      };
      return A2($Array.map,populateAdjacentMines,tiles);
   };
   var addMines = F3(function (randomSeed,numberOfMines,tiles) {
      var insertMines = F2(function (listOfPositions,tiles) {
         insertMines: while (true) {
            var _p0 = listOfPositions;
            if (_p0.ctor === "[]") {
                  return tiles;
               } else {
                  var _p2 = _p0._0;
                  var currentTile = A2($Array.get,_p2,tiles);
                  var _p1 = currentTile;
                  if (_p1.ctor === "Just") {
                        var _v2 = _p0._1,
                        _v3 = A3($Array.set,_p2,_U.update(_p1._0,{isMine: true}),tiles);
                        listOfPositions = _v2;
                        tiles = _v3;
                        continue insertMines;
                     } else {
                        return tiles;
                     }
               }
      ...
      var bombPositionGenerator = A2($Random.list,
      numberOfMines,
      A2($Random.$int,0,$Array.length(tiles)));
      var _p3 = A2($Random.generate,
      bombPositionGenerator,
      $Random.initialSeed(randomSeed));
      var bombPositions = _p3._0;
      return A2(insertMines,bombPositions,tiles);
   });
   var onRightClick = F2(function (address,message) {
      return A4($Html$Events.onWithOptions,
      "contextmenu",
      _U.update($Html$Events.defaultOptions,{preventDefault: true}),
      $Json$Decode.value,
      function (_p4) {
         return A2($Signal.message,address,message);
      });
   });
   var mark = F2(function (tileId,board) {
      return _U.update(board,
      {tiles: A2($List.map,
      function (t) {
         return _U.eq(t.id,tileId) ? $Minesweeper$Tile.mark(t) : t;
      },
      board.tiles)});
   });
   var determineIfFullyExposed = function (board) {
      return A2($List.all,
      function (tile) {
         return tile.isExposed || tile.isMarked;
      },
      board.tiles);
   };
   var expose = F2(function (tileId,board) {
      return _U.update(board,
      {tiles: A2($List.map,
      function (t) {
         return _U.eq(t.id,tileId) ? $Minesweeper$Tile.expose(t) : t;
      },
      board.tiles)});
   });
   var exposeAll = function (board) {
      return _U.update(board,
      {tiles: A2($List.map,$Minesweeper$Tile.expose,board.tiles)});
   };
   var toGrid = function (board) {
      var partition = function (list) {
         return _U.eq($List.length(list),
         board.size) ? _U.list([list]) : A2($Basics._op["++"],
         _U.list([A2($List.take,board.size,list)]),
         partition(A2($List.drop,board.size,list)));
      };
      return partition(board.tiles);
   };
   var create = F3(function (s,numberOfMines,randomSeed) {
      return {hitMine: false
             ,isFullyExposed: false
             ,size: s
             ,tiles: $Array.toList(addAdjacentMineValues(A3(addMines,
             randomSeed,
             numberOfMines,
             A2($Array.initialize,s * s,$Minesweeper$Tile.$new))))};
   });
   var update = F2(function (action,board) {
      var _p5 = action;
      if (_p5.ctor === "Click") {
            var _p6 = _p5._0;
            if (_p6.isMine) {
                  var exposedBoard = exposeAll(board);
                  return {ctor: "_Tuple2"
                         ,_0: _U.update(exposedBoard,{hitMine: true})
                         ,_1: $Effects.none};
               } else {
                  var updatedBoard = A2(expose,_p6.id,board);
                  return {ctor: "_Tuple2"
                         ,_0: _U.update(updatedBoard,
                         {isFullyExposed: determineIfFullyExposed(updatedBoard)})
                         ,_1: $Effects.none};
               }
         } else {
            return {ctor: "_Tuple2"
                   ,_0: A2(mark,_p5._0.id,board)
                   ,_1: $Effects.none};
         }
   });
   ...
   var view = F2(function (address,board) {
      var displayTile = function (tile) {
         return A2($Html.td,
         _U.list([$Html$Attributes.$class($Minesweeper$Tile.classFor(tile))
                 ,A2($Html$Events.onClick,address,Click(tile))
                 ,A2(onRightClick,address,Mark(tile))]),
         _U.list([$Html.text($Minesweeper$Tile.textFor(tile))]));
      };
      var displayRow = function (row) {
         return A2($Html.tr,
         _U.list([]),
         A2($List.map,displayTile,row));
      };
      return A2($Html.table,
      _U.list([]),
      A2($List.map,displayRow,toGrid(board)));
   });
   var Model = F4(function (a,b,c,d) {
      return {size: a,tiles: b,hitMine: c,isFullyExposed: d};
   });
   return _elm.Minesweeper.Board.values = {_op: _op
                                          ,Model: Model
                                          ,Click: Click
                                          ,Mark: Mark
                                          ,update: update
                                          ...
                                          ,exposeAll: exposeAll
                                          ,expose: expose
                                          ,determineIfFullyExposed: determineIfFullyExposed
                                          ,mark: mark
                                          ,onRightClick: onRightClick
                                          ,addMines: addMines
                                          ,addAdjacentMineValues: addAdjacentMineValues};
};
Elm.Minesweeper = Elm.Minesweeper || {};
Elm.Minesweeper.Game = Elm.Minesweeper.Game || {};
Elm.Minesweeper.Game.make = function (_elm) {
   "use strict";
   _elm.Minesweeper = _elm.Minesweeper || {};
   _elm.Minesweeper.Game = _elm.Minesweeper.Game || {};
   if (_elm.Minesweeper.Game.values)
   return _elm.Minesweeper.Game.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Minesweeper$Board = Elm.Minesweeper.Board.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var boardFor = F2(function (difficulty,randomSeed) {
      var _p0 = difficulty;
      switch (_p0.ctor)
      {case "Beginner": return A3($Minesweeper$Board.create,
           9,
           10,
           randomSeed);
         case "Intermediate": return A3($Minesweeper$Board.create,
           16,
           40,
           randomSeed);
         default: return A3($Minesweeper$Board.create,22,99,randomSeed);}
   });
   var initial = function (startTime) {
      return {startTime: startTime
             ,board: $Maybe.Nothing
             ,outcome: $Maybe.Nothing};
   };
   var Won = {ctor: "Won"};
   var Lost = {ctor: "Lost"};
   var Advanced = {ctor: "Advanced"};
   var Intermediate = {ctor: "Intermediate"};
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2"
         ...
         case "Select": return {ctor: "_Tuple2"
                               ,_0: _U.update(model,
                               {board: $Maybe.Just(A2(boardFor,_p2._0,model.startTime))})
                               ,_1: $Effects.none};
         default: var _p3 = model.board;
           if (_p3.ctor === "Just") {
                 var _p4 = A2($Minesweeper$Board.update,_p2._0,_p3._0);
                 var board = _p4._0;
                 var effects = _p4._1;
                 var updatedModel = board.hitMine ? _U.update(model,
                 {board: $Maybe.Just(board)
                 ,outcome: $Maybe.Just(Lost)}) : board.isFullyExposed ? _U.update(model,
                 {board: $Maybe.Just(board)
                 ,outcome: $Maybe.Just(Won)}) : _U.update(model,
                 {board: $Maybe.Just(board)});
                 return {ctor: "_Tuple2"
                        ,_0: updatedModel
                        ,_1: A2($Effects.map,UpdateBoard,effects)};
              } else {
   ...
   var view = F2(function (address,model) {
      var boardHtml = A2($Maybe.map,
      $Minesweeper$Board.view(A2($Signal.forwardTo,
      address,
      UpdateBoard)),
      model.board);
      var toHtml = function (outcome) {
         return A2($Html.div,
         _U.list([$Html$Attributes.$class("outcome")]),
         _U.list([$Html.text($Basics.toString(outcome))]));
      };
      var outcomeHtml = A2($Maybe.withDefault,
      $Html.text(""),
      A2($Maybe.map,toHtml,model.outcome));
      ...
      function (_p5) {
         ...
         address,
         Select(translateDifficulty(_p5)));
      })]),
      _U.list([A2($Html.option,
              _U.list([]),
              _U.list([$Html.text("Select a difficulty...")]))
              ,A2($Html.option,_U.list([]),_U.list([$Html.text("Beginner")]))
              ,A2($Html.option,
              _U.list([]),
              _U.list([$Html.text("Intermediate")]))
              ,A2($Html.option,
              _U.list([]),
              _U.list([$Html.text("Advanced")]))]))]));
      var htmlElements = _U.list([A2($Maybe.withDefault,
                                 controlsHtml,
                                 boardHtml)
                                 ,outcomeHtml]);
   ...
   var Model = F3(function (a,b,c) {
      return {startTime: a,outcome: b,board: c};
   });
   return _elm.Minesweeper.Game.values = {_op: _op
                                         ,Model: Model
                                         ,NoOp: NoOp
                                         ,Select: Select
...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Minesweeper$Game = Elm.Minesweeper.Game.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var init = {ctor: "_Tuple2"
              ,_0: $Minesweeper$Game.initial(startTime)
              ,_1: $Effects.none};
   var app = $StartApp.start({init: init
                             ,view: $Minesweeper$Game.view
                             ,update: $Minesweeper$Game.update
                             ,inputs: _U.list([])});
   var main = app.html;
