module Minesweeper.Game exposing (..)
...
type Action
    = NoOp
    | Select Difficulty
    | UpdateBoard Minesweeper.Board.Action
        ...
        UpdateBoard action ->
            case model.board of
                Just board ->
                    let
                        ( board, effects ) =
                            Minesweeper.Board.update action board

                        updatedModel =
                            if board.hitMine then
                                { model | board = Just board, outcome = Just Lost }
                            else if board.isFullyExposed then
...
view address model =
    let
        ...
        toHtml : Outcome -> Html
        toHtml outcome =
            div [ class "outcome" ] [ outcome |> toString |> text ]

        outcomeHtml =
            model.outcome |> Maybe.map toHtml |> Maybe.withDefault (text "")

        boardHtml =
            model.board |> Maybe.map (Minesweeper.Board.view (Signal.forwardTo address UpdateBoard))

        htmlElements =
            [ Maybe.withDefault controlsHtml boardHtml, outcomeHtml ]
    in
        div [] htmlElements
